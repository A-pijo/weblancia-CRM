    ...
    <div className="space-y-4">
      ...
      <div className="overflow-x-auto rounded-xl border border-admin-border/50 bg-[#141414]">
        <table className="w-full text-sm">
          ...
          <tbody>
            {items.map((service) => (
              <tr key={service.id} className="border-b border-admin-border/20 hover:bg-admin-surface/20 transition-colors">
                ...
                <td className="px-3 py-3">
                  ...
                  <p className="text-xs text-admin-text-tertiary mt-0.5 truncate max-w-[200px]">{service.description}</p>
                </td>
                <td className="px-3 py-3 hidden md:table-cell">
                  <span className="text-xs text-admin-text-secondary">{service.category.title}</span>
                </td>
                <td className="px-3 py-3 text-center hidden sm:table-cell">
                  <span className="text-xs text-admin-text-tertiary">{service.displayOrder}</span>
                </td>
                <td className="px-3 py-3 text-center hidden sm:table-cell">
                  {service.isFeatured ? (
                ...
                <td className="px-3 py-3 text-center">
                  ...
                  >
                    <span className={cn("w-1.5 h-1.5 rounded-full", service.isActive ? "bg-admin-success" : "bg-admin-text-muted")} />
                    {service.isActive ? "Published" : "Draft"}
                  </button>
                </td>
                <td className="px-3 py-3 text-right">
                  <div className="flex items-center justify-end gap-1">
                    <button type="button" onClick={() => onEdit(service.id)} className="p-1.5 rounded text-admin-text-secondary hover:text-admin-text-primary hover:bg-admin-surface transition-colors" aria-label="Edit">
                      <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
                    </button>
                    <a href={`/services/${service.slug}`} target="_blank" rel="noopener noreferrer" className="p-1.5 rounded text-admin-text-secondary hover:text-admin-text-primary hover:bg-admin-surface transition-colors" aria-label="Preview">
                      <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" /><circle cx="12" cy="12" r="3" /></svg>
                    </a>
                    <button type="button" onClick={() => onDuplicate(service.id)} className="p-1.5 rounded text-admin-text-secondary hover:text-admin-text-primary hover:bg-admin-surface transition-colors" aria-label="Duplicate">
                    ...
                    <button type="button" onClick={() => onDelete(service.id)} className="p-1.5 rounded text-admin-text-secondary hover:text-admin-danger hover:bg-admin-danger/10 transition-colors" aria-label="Delete">
            ...
            {items.length === 0 && (
              <tr>
                <td colSpan={7} className="px-3 py-12 text-center text-admin-text-secondary text-sm">No services found</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between">
          <p className="text-xs text-admin-text-tertiary">{total} services total</p>
