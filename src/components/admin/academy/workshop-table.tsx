...
export function WorkshopTable({
  ...
  onPageChange, onSearch, onCategoryFilter, onStatusFilter,
    ...
    <div className="space-y-4">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-admin-border/30">
              ...
              <th className="text-left font-medium text-admin-text-secondary py-3 px-3 hidden sm:table-cell">Date</th>
              <th className="text-right font-medium text-admin-text-secondary py-3 px-3 hidden sm:table-cell">Price</th>
              <th className="text-center font-medium text-admin-text-secondary py-3 px-3 hidden sm:table-cell">Status</th>
          ...
          <tbody>
            {items.map((ws) => (
              <tr key={ws.id} className="border-b border-admin-border/20 hover:bg-admin-surface/20 transition-colors">
                <td className="px-3 py-3">
                  <input type="checkbox" checked={selected.has(ws.id)} onChange={() => toggleSelect(ws.id)} className="rounded border-admin-text-muted bg-admin-surface" />
                </td>
                <td className="px-3 py-3">
                  <button type="button" onClick={() => onEdit(ws.id)} className="text-admin-text-primary hover:text-admin-accent transition-colors text-left font-medium">{ws.title}</button>
                </td>
                <td className="px-3 py-3 hidden md:table-cell"><span className="text-xs text-admin-text-secondary">{ws.category?.title ?? "—"}</span></td>
                <td className="px-3 py-3 hidden lg:table-cell"><span className="text-xs text-admin-text-secondary">{ws.instructor ?? "—"}</span></td>
                <td className="px-3 py-3 hidden sm:table-cell"><span className="text-xs text-admin-text-secondary">{ws.date ? new Date(ws.date).toLocaleDateString() : "—"}</span></td>
                <td className="px-3 py-3 text-right hidden sm:table-cell">
                  <span className="text-xs text-admin-text-secondary">
                    {ws.price != null ? `$${ws.price}` : "—"}
                    {ws.price != null && ws.price === 0 && <span className="text-admin-success ml-1">Free</span>}
                  </span>
                </td>
                <td className="px-3 py-3 text-center hidden sm:table-cell">
                  <span className={cn("inline-flex px-2 py-0.5 rounded text-xs font-medium",
                    ws.status === "Upcoming" ? "text-blue-400 bg-blue-500/10" :
                    ws.status === "Ongoing" ? "text-admin-success bg-admin-success/10" :
                    ws.status === "Completed" ? "text-admin-text-secondary bg-admin-surface" :
                    ws.status === "Cancelled" ? "text-admin-danger bg-admin-danger/10" : "text-admin-text-secondary bg-admin-surface"
                  )}>{ws.status ?? "—"}</span>
                </td>
                <td className="px-3 py-3 text-center">
                  <button type="button" onClick={() => onToggle(ws.id, !ws.isPublished)}
                    className={cn("inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium transition-colors",
                      ws.isPublished ? "bg-admin-success/10 text-admin-success hover:bg-admin-success/15" : "bg-admin-surface text-admin-text-secondary hover:bg-admin-surface"
                    )}>
                    <span className={cn("w-1.5 h-1.5 rounded-full", ws.isPublished ? "bg-admin-success" : "bg-admin-text-muted")} />
                    {ws.isPublished ? "Published" : "Draft"}
                  </button>
                </td>
                <td className="px-3 py-3 text-right">
                  <div className="flex items-center justify-end gap-1">
                    <button type="button" onClick={() => onEdit(ws.id)} className="p-1.5 rounded text-admin-text-secondary hover:text-admin-text-primary hover:bg-admin-surface transition-colors" aria-label="Edit">
                      <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
                    </button>
                    <a href={`/academy/workshops/${ws.slug}`} target="_blank" rel="noopener noreferrer" className="p-1.5 rounded text-admin-text-secondary hover:text-admin-text-primary hover:bg-admin-surface transition-colors" aria-label="Preview">
                      <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" /><circle cx="12" cy="12" r="3" /></svg>
                    </a>
                    <button type="button" onClick={() => onDelete(ws.id)} className="p-1.5 rounded text-admin-text-secondary hover:text-admin-danger hover:bg-admin-danger/10 transition-colors" aria-label="Delete">
            ...
            {items.length === 0 && <tr><td colSpan={9} className="px-3 py-12 text-center text-admin-text-secondary text-sm">No workshops found</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between">
          <p className="text-xs text-admin-text-tertiary">{total} workshops total</p>
